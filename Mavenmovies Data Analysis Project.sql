...
-- List all employees (staff) and the films they have rented, where the rental is still not returned
SELECT * FROM staff;
...
-- List all customers and any films they rented, including those who have not rented 
-- any films (use LEFT JOIN).
...
SELECT category.name, COUNT(DISTINCT rental.customer_id) AS unique_customer_count
FROM rental
JOIN inventory ON rental.inventory_id = inventory.inventory_id
